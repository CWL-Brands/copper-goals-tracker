...
const domain = 'kanvabotanicals.com';

function validateEmail(email: string) {
  if (!emailRegex.test(email)) return 'Invalid email format';
  const lower = email.toLowerCase();
  if (!lower.endsWith(`@${domain}`)) return `Email must be @${domain}`;
  return null;
}
    ...
    <div className="flex items-center justify-center min-h-screen bg-gray-50 p-4">
      <div className="bg-white w-full max-w-md rounded-xl shadow-sm p-6">
        ...
        <form onSubmit={onSubmit} className="space-y-4">
          ...
          <p className="text-xs text-gray-500 text-center">Use your @{domain} email</p>
        </form>
      </div>
    </div>
  );
