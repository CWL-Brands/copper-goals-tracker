...
export async function GET(req: NextRequest) {
  try {
    ...
    const types = ['talk_time','email_quantity','lead_progression_a','lead_progression_b','lead_progression_c','new_sales_wholesale','new_sales_distribution'];
    const series: Record<string, { date: string; value: number }[]> = {} as any;
    types.forEach((t) => { series[t] = days.map((day) => ({ date: day, value: Number(byDay[day]?.[t] || 0) })); });
